...
def get_coco_filename(img_info):
    # 1) file_name이 있으면 그대로 사용
    if 'file_name' in img_info and img_info['file_name']:
        return img_info['file_name']
    # 2) coco_url 있으면 basename 사용
    if 'coco_url' in img_info and img_info['coco_url']:
        return os.path.basename(img_info['coco_url'])
    # 3) 마지막 수단: COCO 규칙 + 네 디렉토리 구조 반영
    #    LVIS_IMG_ROOT = ".../datasets/val2017" 이므로
    #    "val2017/000000xxxxxx.jpg" 를 반환해야 최종 경로가 ".../val2017/val2017/..."가 됨
...
def main():
    ...
    inferencer.model.test_cfg.max_per_img = 1000
    ...
    for img_id in tqdm(split_img_ids):
        ...
        labels = np.array(results['predictions'][0]['labels'])
        scores = np.array(results['predictions'][0]['scores'], dtype=float)
        bboxes = np.array(results['predictions'][0]['bboxes'], dtype=float)
        ...
        for label_idx, score, bbox in zip(results['predictions'][0]['labels'], 
            ...
            if 'yes' in ans_lower:
                lvis_category_name = lvis_category_names[label_idx]
                cat_id = name2id[lvis_category_name]
