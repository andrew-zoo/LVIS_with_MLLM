...
def parse_args():
    ...
    # ↓ 추가: 실행 스크립트에서 mpi 설정 가능
    parser.add_argument('--max-per-img', type=int, default=100, help='Max detections per image (MPI)')
...
def main():
    ...
    # max_per_img 적용 (로그 추가 출력 없음, 진행바 유지)
    mpi = int(call_args.pop('max_per_img', 100))  # det_kwargs로 넘어가지 않게 pop
    if hasattr(inferencer.model, 'test_cfg') and hasattr(inferencer.model.test_cfg, 'max_per_img'):
        inferencer.model.test_cfg.max_per_img = mpi
        
    print(f"[cfg] max_per_img (mpi) = {mpi}")  # ← 확인용 한 줄
